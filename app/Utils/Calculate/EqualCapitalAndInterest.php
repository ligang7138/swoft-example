...
/**
1. 等本等息方式还款： 每期还款金额=本金*（1+年利率)/总期数
例： 借款12000，1年12期，年利率10%
    每月还本金=12000/12=1000，

    每月还利息=12000*10%/12=100，

    每月共还本息=1100

    总利息=12000*10%=1200
 */
class EqualCapitalAndInterest implements CalculateBoxInterface
{
    public static function result(array $params=[]): array
    {
        $capital = $params['capital'];//剩余本金
        $totalInterest = round($params['capital']*$params['monthRate']*$params['period'],8);//总利息
	    $intrest = $totalInterest;//剩余利息
	    $return = $params;
        ...
        for ($i=1;$i<=$params['period'];$i++){
        	if($params['is_benchmark']){
		        if($i!=$params['period']){
			        ...
			        $monthCapital = $params['capital']/$params['period'];//当月本金
			        $capital = round($capital-$monthCapital,2);//剩余本金
			        $intrest = round($intrest - $monthInterest,2);//剩余利息
		        }else{
	...
	public static function clearReturnResult(array $params = []): array
	{
		...
		$total_return_period = intval(count($factReturnList)) + count($clear_return_info);//合同还款总期数
		if($factReturnList){
		...
		//结清减免额，不含咨询费和违约金
		$info['clear_return_derate_amt']  = round($total_amount - $info['curr_month_should_amt'],2);
